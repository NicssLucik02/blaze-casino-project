export function createStarsManager(width: number, height: number) {
  ...
  const drawStars = (
    ...
    const normalizedScrollX = scrollX % width;
    const normalizedScrollY = scrollY % height;

    for (let gridX = -1; gridX <= 1; gridX++) {
      for (let gridY = -1; gridY <= 1; gridY++) {
        baseStars.forEach(star => {
          const x = star.x + gridX * width - normalizedScrollX;
          const y = star.y + gridY * height + normalizedScrollY;
        ...
        baseSparkles.forEach(sparkle => {
          const x = sparkle.x + gridX * width - normalizedScrollX * 1.5;
          const y = sparkle.y + gridY * height + normalizedScrollY * 1.5;
